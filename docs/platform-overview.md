...
- 分類：時尚穿搭、美妝保養、保健品、食品零食、3C電器、生活雜貨、運動戶外、公仔玩具、書籍文具、寵物用品、文化紀念品、汽機車用品、母嬰用品、珠寶首飾、明星偶像、其他
- 型號（選填）
- 封面圖片（選填）
...
3. 畫面立即顯示「**審核中**」，文案：「我們已收到你的驗證，管理員審核通過後會以通知告知你。」
   - **返回**：回到社群連結列表，**保留**這筆待審申請（背景審核 / IG 自動掃描照常進行）
   - **取消**：刪除這筆驗證申請並回到列表
4. 待審期間，列表該平台列的按鈕顯示「**審核中**」，可點回審核中卡片

**Instagram** 比 Threads 多一道「自動驗證」：按「我已傳送」後背景每 10 秒掃管理員 IG 收件匣比對驗證碼，命中即秒過（標記來源 🤖 自動）；連掃 5 次（約 50 秒）未中才升級為管理員人工待審。**Threads** 為純人工審核（送出即進入待審）。

> 審核狀態還原：重新整理頁面時會向後端查詢進行中的申請，`created` → 回到傳送驗證碼畫面、`sent`/`pending` → 回到「審核中」、`rejected` → 顯示驗證未通過可重試。
...
2. **Instagram** 採「自動為主、人工為輔」：賣家按「我已傳送」後系統自動掃收件匣比對驗證碼，命中即秒過；連掃 5 次（約 50 秒）掃不到才落到此處人工待審
3. **Threads** 為純人工審核
...
| 路由 | 說明 |
|------|------|
| `/` | 首頁（頂部橫式 banner 圖片輪播主視覺，自動輪播＋圓點＋左右箭頭，圖片未上前以品牌色漸層佔位；16 個商品分類 + 熱門商品橫滑 + 即將出發連線橫滑 + 成為賣家 CTA；async RSC，熱門商品依 `product_views` 排序，無瀏覽以最新遞補；任一排無資料則不渲染。��尋入口由 Header 提供） |
