    ...
    >
      {items.map(({ value, label, count }) => (
        <TabsTrigger key={value} value={value} className="group">
          {label}
          {count !== undefined && (
            <span className="ml-0.5 text-[11px] font-semibold px-1.5 py-px rounded-full bg-background text-muted-foreground group-data-active:bg-white/25 group-data-active:text-white md:text-[12px]">
              {count}
            </span>
          )}
        </TabsTrigger>
