...
**搜尋：**
- 商品搜尋邏輯由買家瀏覽頁「商品」分頁與賣家新增代購時的商品搜尋共用
- 使用 PostgreSQL `pg_trgm` 模糊比對，支援錯別字容錯（短字串效果有限）
- 搜尋欄位 `search_text` 由觸��器自動維護，內容為 `katakana_to_hiragana(lower(name) || ' ' || lower(array_to_string(aliases, ' ')) || ' ' || lower(model_number))`（商品名稱 + 別名 + 型號）；片假名統一轉為平假名，使「ソニー」與「そにー」可互相匹配
- 品牌名稱（`brands.name`）亦納入搜尋比對，同樣套用片假名正規化
- 買家瀏覽頁的「商品」分頁用 `search_product_ids`（比對商品 `search_text` + 品牌）；「代購」分頁用 `search_listing_ids`，比對「代購標題 `listings.title` OR 連結商品 + 品牌」的聯集——兩分頁搜尋彼此獨立，代購標題含關鍵字即可被搜到（即使連結商品不含）
...
- 全站搜尋（header `q`）透過 `search_connection_ids` RPC，比對 `title`、`description`、`locations_text` 以及連線掛的品牌名（`connection_brands` → `brands.name`）的聯集，並套用 katakana→hiragana 正規化（與商品 / 代購搜尋一致）
...
- 已登入使用者可從此頁許願（`/wishes/new`）：流程與賣家新增代購一致，先搜尋既有商品，找不到再新增商品，最後填寫許願內容送出（避免重複建立商品）
...
- **建立新��品**：填寫名稱（必填）、品牌（選填，可直接新增）、型號（選填）、分類（選填）、商品圖片（必填，至少 1 張、最多 5 張，第一張為封面，可用上下箭頭排序）
...
| `/wishes/new` | 許願（先搜尋既有商品或新增，再填許願內容） |
...
> 2026-06-27 重整：選擇器改用 `data-testid`（共用 `tests/helpers/locators.ts`，禁用 CSS class）；註冊／重設密碼用 `auth.admin.generateLink` 模擬點信、不靠真信箱；onboarding/become-seller 等需乾淨帳號的測試用臨時帳號（`e2e-tmp-*`，測後刪）。查 schema 一律以真實 DB 為準（`server/db/types.ts` 手寫已過時）。
...
**託管：** Vercel，GitHub 連動（`pinxian0802/Kozukase`）。production 分支為 `master`，`dev` 為開發 / preview 分支。

**Function Region：** `vercel.json` 設 `"regions": ["icn1"]`（首爾）。**必須與 Supabase 同城**：Supabase 專案在 `ap-northeast-2`（首爾），SSR / tRPC 每次請求會多次查 DB，若 function 與 DB 跨區（例如預設的 `iad1` 美東）會讓每個查詢跨太平洋來回、TTFB 暴增。

> 2026-06-23 將 region 從 `iad1`（美東）改為 `icn1`（首爾），首頁暖機 TTFB 從 ~1.1s 降到 ~0.3s（約降 7 成）。驗證方式：`curl -sD - https://www.kozukase.com/ | grep x-vercel-id`，第二段應為 `icn1`。

**Cron：** `vercel.json` 定義一個每日 cron（`/api/cron/expire-daily`，`0 16 * * *` = 台灣凌晨 00:00），同時下架過期商品與結束過期媒合。媒合的 `end_date` 以台灣當天日期判斷。

**圖片儲存與孤兒清理：** 圖片實體檔放在 Cloudflare R2（`images/{purpose}/users/{userId}/...`）。孤兒檔（R2 有檔、但資料庫無任何引用）的清理分兩塊：

- **後台手動掃描（A）**：`/admin/storage` 頁(`storage.scanOrphanImages` / `deleteOrphanImages`，管理員限定)列舉 R2 與資料庫所有被引用的 key 比對，列出「沒人用且上傳超過 24 小時」的孤兒供管理員確認後刪除。刪除前會重新比對取交集，避免競態誤刪。被引用 key 來源：`product_images`／`listing_images`／`connection_images` 的 `r2_key`+`thumbnail_r2_key`、`home_banners.image_r2_key`、`profiles`／`sellers.avatar_url`、`messages` 的 `image_url`+`context_image_url`。
- **源頭止血（C）**：刪代購／連線、代購／連線換圖時於源頭 best-effort 刪 R2（`listing`／`connection` 的 `delete`、`upload` 的 `confirmListingImages`／`confirmConnectionImages`）。大頭貼、商品圖因跨表共用或軟刪除，不在源頭刪，交由 A 兜底；橫幅刪除原本就有刪 R2。

## 十二、資料存取與 RLS 模型（重要）

平台有三條資料讀取路徑，安全模型各自不同：

1. **tRPC（App 主要路徑）**：`ctx.db` 用 **service-role**（`server/db/client.ts` 的 `getDb()`），**繞過 RLS**。公開瀏覽用 `publicProcedure`（`product/listing/connection/seller` 的 `search/browse/getById/popular` 等），未登入也讀得到。
2. **前端 Realtime（anon key 直連）**：完全依賴 RLS，由 2026-05-20「28 表 ENABLE RLS」（migration `00033`）保護。
3. **SSR 公開頁的 metadata / sitemap**：**必須用 service-role（`getDb()`），不可用 anon 的 `createSupabaseServerClient()`。**

> **踩過的雷（2026-06-28 修正）**：`products`/`sellers`/`listings`/`connections` 等公開瀏覽表的 SELECT policy 只給 `{authenticated}`、沒有 `anon`。詳情頁 `[id]/page.tsx` 的 `generateMetadata`／JSON-LD 與 `app/sitemap.ts` 原本用 anon SSR client，導致**未登入訪客與爬蟲**拿到預設 title、sitemap 缺所有動態網址（內容因走 tRPC service-role 正常）。
>
> **修法**：這些 server-only 的公開讀取改用 `getDb()`（service-role），**不開放 anon RLS**——因為 anon key 是公開的、RLS row policy 不限欄位，開 anon 會讓 `sellers.phone_number`／`ig_user_id` 等 PII 被任何人直接撈走。service-role 只在伺服器端、金鑰不外流。
>
> **新增公開 SSR 讀取時**：一律用 `getDb()` 並自行帶齊「公開可見」過濾（`is_removed=false`／`status='active'`／`is_suspended=false`），別用 anon client。私密表（messages/conversations 等）的 RLS 已用 `auth.uid()` 參與者條件正確保護，不受影響。
